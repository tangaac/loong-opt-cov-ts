	.file	"functs.c"
	.section	.rodata.cst8,"aM",@progbits,8
	.p2align	3, 0x0                          # -- Begin function speedup_test
.LCPI0_0:
	.dword	0x4059000000000000              # double 100
	.text
	.globl	speedup_test
	.p2align	5
	.type	speedup_test,@function
speedup_test:                           # @speedup_test
# %bb.0:
	addi.d	$sp, $sp, -368
	st.d	$ra, $sp, 360                   # 8-byte Folded Spill
	st.d	$fp, $sp, 352                   # 8-byte Folded Spill
	st.d	$s0, $sp, 344                   # 8-byte Folded Spill
	move	$fp, $a0
	pcaddu18i	$ra, %call36(rewind)
	jirl	$ra, $ra, 0
	pcalau12i	$a0, %pc_hi20(.L.str)
	addi.d	$a1, $a0, %pc_lo12(.L.str)
	addi.d	$a2, $sp, 244
	addi.d	$a3, $sp, 24
	move	$a0, $fp
	pcaddu18i	$ra, %call36(__isoc99_fscanf)
	jirl	$ra, $ra, 0
	ori	$s0, $zero, 69
	.p2align	4, , 16
.LBB0_1:                                # =>This Inner Loop Header: Depth=1
	addi.d	$a0, $sp, 144
	addi.d	$a1, $sp, 244
	pcaddu18i	$ra, %call36(strcpy)
	jirl	$ra, $ra, 0
	addi.d	$a0, $sp, 244
	ori	$a1, $zero, 100
	move	$a2, $fp
	pcaddu18i	$ra, %call36(fgets)
	jirl	$ra, $ra, 0
	ld.bu	$a0, $sp, 244
	bne	$a0, $s0, .LBB0_1
# %bb.2:
	pcalau12i	$a0, %pc_hi20(.L.str.1)
	addi.d	$a1, $a0, %pc_lo12(.L.str.1)
	addi.d	$a0, $sp, 144
	addi.d	$a2, $sp, 44
	addi.d	$a3, $sp, 32
	addi.d	$a4, $sp, 16
	pcaddu18i	$ra, %call36(__isoc99_sscanf)
	jirl	$ra, $ra, 0
	pcalau12i	$a0, %pc_hi20(.L.str.2)
	addi.d	$a1, $a0, %pc_lo12(.L.str.2)
	addi.d	$a0, $sp, 244
	addi.d	$a2, $sp, 44
	addi.d	$a3, $sp, 32
	addi.d	$a4, $sp, 8
	pcaddu18i	$ra, %call36(__isoc99_sscanf)
	jirl	$ra, $ra, 0
	ld.d	$a0, $sp, 16
	ld.d	$a1, $sp, 24
	sub.d	$a1, $a0, $a1
	pcalau12i	$a0, %pc_hi20(loop_time)
	st.d	$a1, $a0, %pc_lo12(loop_time)
	pcalau12i	$a0, %pc_hi20(.L.str.3)
	addi.d	$a0, $a0, %pc_lo12(.L.str.3)
	pcaddu18i	$ra, %call36(printf)
	jirl	$ra, $ra, 0
	ld.d	$a1, $sp, 8
	pcalau12i	$a0, %pc_hi20(prog_time)
	st.d	$a1, $a0, %pc_lo12(prog_time)
	pcalau12i	$a0, %pc_hi20(.L.str.4)
	addi.d	$a0, $a0, %pc_lo12(.L.str.4)
	pcaddu18i	$ra, %call36(printf)
	jirl	$ra, $ra, 0
	ld.d	$a0, $sp, 16
	ld.d	$a1, $sp, 24
	sub.d	$a0, $a0, $a1
	srli.d	$a1, $a0, 32
	lu52i.d	$a2, $zero, 1107
	or	$a1, $a1, $a2
	movgr2fr.d	$fa0, $a1
	lu12i.w	$a1, 256
	lu52i.d	$a1, $a1, 1107
	movgr2fr.d	$fa1, $a1
	fsub.d	$fa0, $fa0, $fa1
	lu12i.w	$a1, 275200
	ld.d	$a3, $sp, 8
	bstrins.d	$a0, $a1, 63, 32
	movgr2fr.d	$fa2, $a0
	fadd.d	$fa0, $fa2, $fa0
	srli.d	$a0, $a3, 32
	or	$a0, $a0, $a2
	movgr2fr.d	$fa2, $a0
	fsub.d	$fa1, $fa2, $fa1
	bstrins.d	$a3, $a1, 63, 32
	pcalau12i	$a0, %pc_hi20(.LCPI0_0)
	fld.d	$fa2, $a0, %pc_lo12(.LCPI0_0)
	movgr2fr.d	$fa3, $a3
	fadd.d	$fa1, $fa3, $fa1
	fdiv.d	$fa0, $fa0, $fa1
	fmul.d	$fa0, $fa0, $fa2
	movfr2gr.d	$a1, $fa0
	pcalau12i	$a0, %pc_hi20(.L.str.5)
	addi.d	$a0, $a0, %pc_lo12(.L.str.5)
	pcaddu18i	$ra, %call36(printf)
	jirl	$ra, $ra, 0
	move	$a0, $fp
	pcaddu18i	$ra, %call36(rewind)
	jirl	$ra, $ra, 0
	ld.d	$s0, $sp, 344                   # 8-byte Folded Reload
	ld.d	$fp, $sp, 352                   # 8-byte Folded Reload
	ld.d	$ra, $sp, 360                   # 8-byte Folded Reload
	addi.d	$sp, $sp, 368
	ret
.Lfunc_end0:
	.size	speedup_test, .Lfunc_end0-speedup_test
                                        # -- End function
	.globl	imix_test                       # -- Begin function imix_test
	.p2align	5
	.type	imix_test,@function
imix_test:                              # @imix_test
# %bb.0:
	addi.d	$sp, $sp, -368
	st.d	$ra, $sp, 360                   # 8-byte Folded Spill
	st.d	$fp, $sp, 352                   # 8-byte Folded Spill
	st.d	$s0, $sp, 344                   # 8-byte Folded Spill
	st.d	$s1, $sp, 336                   # 8-byte Folded Spill
	move	$fp, $a0
	pcaddu18i	$ra, %call36(rewind)
	jirl	$ra, $ra, 0
	addi.d	$a0, $sp, 236
	ori	$a1, $zero, 100
	move	$a2, $fp
	pcaddu18i	$ra, %call36(fgets)
	jirl	$ra, $ra, 0
	pcalau12i	$a0, %pc_hi20(.L.str)
	addi.d	$a1, $a0, %pc_lo12(.L.str)
	addi.d	$a2, $sp, 236
	addi.d	$a3, $sp, 16
	move	$a0, $fp
	pcaddu18i	$ra, %call36(__isoc99_fscanf)
	jirl	$ra, $ra, 0
	move	$s0, $zero
	ori	$s1, $zero, 69
	.p2align	4, , 16
.LBB1_1:                                # =>This Inner Loop Header: Depth=1
	ld.bu	$a0, $sp, 238
	addi.d	$a0, $a0, -58
	sltui	$a0, $a0, 1
	add.d	$s0, $s0, $a0
	addi.d	$a0, $sp, 136
	addi.d	$a1, $sp, 236
	pcaddu18i	$ra, %call36(strcpy)
	jirl	$ra, $ra, 0
	addi.d	$a0, $sp, 236
	ori	$a1, $zero, 100
	move	$a2, $fp
	pcaddu18i	$ra, %call36(fgets)
	jirl	$ra, $ra, 0
	ld.bu	$a0, $sp, 236
	bne	$a0, $s1, .LBB1_1
# %bb.2:
	pcalau12i	$a0, %pc_hi20(.L.str.1)
	addi.d	$a1, $a0, %pc_lo12(.L.str.1)
	addi.d	$a0, $sp, 136
	addi.d	$a2, $sp, 36
	addi.d	$a3, $sp, 24
	addi.d	$a4, $sp, 8
	pcaddu18i	$ra, %call36(__isoc99_sscanf)
	jirl	$ra, $ra, 0
	move	$a0, $fp
	pcaddu18i	$ra, %call36(rewind)
	jirl	$ra, $ra, 0
	move	$a0, $s0
	ld.d	$s1, $sp, 336                   # 8-byte Folded Reload
	ld.d	$s0, $sp, 344                   # 8-byte Folded Reload
	ld.d	$fp, $sp, 352                   # 8-byte Folded Reload
	ld.d	$ra, $sp, 360                   # 8-byte Folded Reload
	addi.d	$sp, $sp, 368
	ret
.Lfunc_end1:
	.size	imix_test, .Lfunc_end1-imix_test
                                        # -- End function
	.globl	find_hard_raws                  # -- Begin function find_hard_raws
	.p2align	5
	.type	find_hard_raws,@function
find_hard_raws:                         # @find_hard_raws
# %bb.0:
	pcalau12i	$a0, %got_pc_hi20(hard_raw_list)
	ld.d	$a0, $a0, %got_pc_lo12(hard_raw_list)
	ld.d	$a0, $a0, 0
	bnez	$a0, .LBB2_4
.LBB2_1:                                # %._crit_edge
	ret
	.p2align	4, , 16
.LBB2_2:                                # %.lr.ph._crit_edge
                                        #   in Loop: Header=BB2_4 Depth=1
	ld.d	$a0, $a0, 0
.LBB2_3:                                #   in Loop: Header=BB2_4 Depth=1
	beqz	$a0, .LBB2_1
.LBB2_4:                                # %.lr.ph
                                        # =>This Inner Loop Header: Depth=1
	ld.d	$a1, $a0, 48
	ld.d	$a2, $a0, 32
	bgeu	$a2, $a1, .LBB2_2
# %bb.5:                                #   in Loop: Header=BB2_4 Depth=1
	ld.d	$a1, $a0, 8
	ld.d	$a0, $a0, 0
	beqz	$a1, .LBB2_3
# %bb.6:                                #   in Loop: Header=BB2_4 Depth=1
	st.d	$a0, $a1, 0
	b	.LBB2_3
.Lfunc_end2:
	.size	find_hard_raws, .Lfunc_end2-find_hard_raws
                                        # -- End function
	.globl	specul_time_o                   # -- Begin function specul_time_o
	.p2align	5
	.type	specul_time_o,@function
specul_time_o:                          # @specul_time_o
# %bb.0:
	addi.d	$sp, $sp, -80
	st.d	$ra, $sp, 72                    # 8-byte Folded Spill
	st.d	$fp, $sp, 64                    # 8-byte Folded Spill
	st.d	$s0, $sp, 56                    # 8-byte Folded Spill
	st.d	$s1, $sp, 48                    # 8-byte Folded Spill
	st.d	$s2, $sp, 40                    # 8-byte Folded Spill
	st.d	$s3, $sp, 32                    # 8-byte Folded Spill
	st.d	$s4, $sp, 24                    # 8-byte Folded Spill
	st.d	$s5, $sp, 16                    # 8-byte Folded Spill
	st.d	$s6, $sp, 8                     # 8-byte Folded Spill
	move	$s1, $a5
	move	$s2, $a4
	move	$s0, $a3
	move	$fp, $a2
	move	$s3, $a1
	blez	$a1, .LBB3_3
# %bb.1:                                # %.lr.ph.preheader
	addi.d	$a1, $a0, 48
	move	$a2, $s3
	.p2align	4, , 16
.LBB3_2:                                # %.lr.ph
                                        # =>This Inner Loop Header: Depth=1
	ld.d	$a3, $a1, -16
	ld.d	$a4, $a1, -24
	sub.d	$a3, $a3, $a4
	st.d	$a3, $a1, -8
	st.d	$zero, $a1, 0
	addi.d	$a2, $a2, -1
	addi.d	$a1, $a1, 56
	bnez	$a2, .LBB3_2
.LBB3_3:                                # %.preheader81
	pcalau12i	$a1, %got_pc_hi20(hard_raw_list)
	ld.d	$a1, $a1, %got_pc_lo12(hard_raw_list)
	ld.d	$a1, $a1, 0
	beqz	$a1, .LBB3_8
# %bb.4:                                # %.lr.ph87.preheader
	move	$s4, $zero
	ori	$a2, $zero, 56
	b	.LBB3_6
	.p2align	4, , 16
.LBB3_5:                                #   in Loop: Header=BB3_6 Depth=1
	ld.d	$a1, $a1, 0
	beqz	$a1, .LBB3_9
.LBB3_6:                                # %.lr.ph87
                                        # =>This Inner Loop Header: Depth=1
	ld.w	$a3, $a1, 24
	ld.d	$a4, $a1, 32
	ld.w	$a5, $a1, 40
	mul.d	$a3, $a3, $a2
	add.d	$a3, $a0, $a3
	ld.d	$a6, $a3, 48
	mul.d	$a3, $a5, $a2
	add.d	$a3, $a0, $a3
	ld.d	$a5, $a3, 48
	add.d	$a4, $a6, $a4
	bgeu	$a5, $a4, .LBB3_5
# %bb.7:                                #   in Loop: Header=BB3_6 Depth=1
	st.d	$a4, $a3, 48
	addi.d	$s4, $s4, 1
	b	.LBB3_5
.LBB3_8:
	move	$s4, $zero
.LBB3_9:                                # %.preheader
	blez	$s3, .LBB3_12
# %bb.10:                               # %.lr.ph89.preheader
	ori	$a2, $zero, 1
	bstrpick.d	$a1, $s3, 30, 1
	bne	$s3, $a2, .LBB3_13
# %bb.11:
	move	$a2, $zero
	b	.LBB3_16
.LBB3_12:                               # %._crit_edge
	ld.d	$s5, $a0, 40
	b	.LBB3_25
.LBB3_13:                               # %vector.ph
	slli.d	$a2, $a1, 1
	addi.d	$a3, $a0, 96
	move	$a4, $a2
	.p2align	4, , 16
.LBB3_14:                               # %vector.body
                                        # =>This Inner Loop Header: Depth=1
	ld.d	$a5, $a3, -56
	ld.d	$a6, $a3, 0
	ld.d	$a7, $a3, -48
	ld.d	$t0, $a3, 8
	add.d	$a5, $a7, $a5
	add.d	$a6, $t0, $a6
	st.d	$a5, $a3, -56
	st.d	$a6, $a3, 0
	addi.d	$a4, $a4, -2
	addi.d	$a3, $a3, 112
	bnez	$a4, .LBB3_14
# %bb.15:                               # %middle.block
	beq	$a2, $s3, .LBB3_18
.LBB3_16:                               # %.lr.ph89.preheader122
	ori	$a3, $zero, 56
	mul.d	$a3, $a2, $a3
	add.d	$a3, $a3, $a0
	addi.d	$a3, $a3, 40
	sub.d	$a2, $s3, $a2
	.p2align	4, , 16
.LBB3_17:                               # %.lr.ph89
                                        # =>This Inner Loop Header: Depth=1
	ld.d	$a4, $a3, 0
	ld.d	$a5, $a3, 8
	add.d	$a4, $a5, $a4
	st.d	$a4, $a3, 0
	addi.d	$a2, $a2, -1
	addi.d	$a3, $a3, 56
	bnez	$a2, .LBB3_17
.LBB3_18:                               # %.lr.ph93.preheader
	ld.d	$s5, $a0, 40
	ori	$a2, $zero, 1
	bne	$s3, $a2, .LBB3_20
# %bb.19:
	move	$a1, $zero
	b	.LBB3_23
.LBB3_20:                               # %vector.ph110
	slli.d	$a1, $a1, 1
	addi.d	$a2, $a0, 96
	move	$a3, $a1
	move	$a4, $s5
	.p2align	4, , 16
.LBB3_21:                               # %vector.body113
                                        # =>This Inner Loop Header: Depth=1
	ld.d	$a5, $a2, -56
	ld.d	$a6, $a2, 0
	sltu	$a7, $s5, $a5
	masknez	$t0, $s5, $a7
	maskeqz	$a5, $a5, $a7
	or	$s5, $a5, $t0
	sltu	$a5, $a4, $a6
	masknez	$a4, $a4, $a5
	maskeqz	$a5, $a6, $a5
	or	$a4, $a5, $a4
	addi.d	$a3, $a3, -2
	addi.d	$a2, $a2, 112
	bnez	$a3, .LBB3_21
# %bb.22:                               # %middle.block117
	sltu	$a2, $a4, $s5
	masknez	$a3, $a4, $a2
	maskeqz	$a2, $s5, $a2
	or	$s5, $a2, $a3
	beq	$a1, $s3, .LBB3_25
.LBB3_23:                               # %.lr.ph93.preheader120
	ori	$a2, $zero, 56
	mul.d	$a2, $a1, $a2
	add.d	$a0, $a2, $a0
	addi.d	$a0, $a0, 40
	sub.d	$a1, $s3, $a1
	.p2align	4, , 16
.LBB3_24:                               # %.lr.ph93
                                        # =>This Inner Loop Header: Depth=1
	ld.d	$a2, $a0, 0
	sltu	$a3, $s5, $a2
	masknez	$a4, $s5, $a3
	maskeqz	$a2, $a2, $a3
	or	$s5, $a2, $a4
	addi.d	$a1, $a1, -1
	addi.d	$a0, $a0, 56
	bnez	$a1, .LBB3_24
.LBB3_25:                               # %._crit_edge94
	pcalau12i	$a0, %pc_hi20(.Lstr)
	addi.d	$a0, $a0, %pc_lo12(.Lstr)
	pcaddu18i	$ra, %call36(puts)
	jirl	$ra, $ra, 0
	add.d	$a0, $s4, $s3
	mul.d	$a0, $a0, $s2
	mul.w	$a1, $s1, $s3
	add.d	$a0, $a0, $a1
	add.d	$s1, $a0, $s5
	pcalau12i	$a0, %pc_hi20(.L.str.7)
	addi.d	$a0, $a0, %pc_lo12(.L.str.7)
	move	$a1, $s1
	pcaddu18i	$ra, %call36(printf)
	jirl	$ra, $ra, 0
	move	$s6, $s0
	bstrins.d	$s6, $zero, 1, 1
	ori	$a0, $zero, 1
	pcalau12i	$s4, %pc_hi20(loop_time)
	lu52i.d	$s3, $zero, 1107
	lu12i.w	$s5, 256
	lu12i.w	$s2, 275200
	beq	$s6, $a0, .LBB3_28
# %bb.26:
	bstrins.d	$s0, $zero, 0, 0
	ori	$a0, $zero, 2
	beq	$s0, $a0, .LBB3_29
.LBB3_27:
	bnez	$fp, .LBB3_30
	b	.LBB3_34
.LBB3_28:
	ld.d	$a0, $s4, %pc_lo12(loop_time)
	srli.d	$a1, $a0, 32
	or	$a1, $a1, $s3
	movgr2fr.d	$fa0, $a1
	lu52i.d	$a1, $s5, 1107
	movgr2fr.d	$fa1, $a1
	fsub.d	$fa0, $fa0, $fa1
	bstrins.d	$a0, $s2, 63, 32
	movgr2fr.d	$fa2, $a0
	fadd.d	$fa0, $fa2, $fa0
	srli.d	$a0, $s1, 32
	or	$a0, $a0, $s3
	movgr2fr.d	$fa2, $a0
	fsub.d	$fa1, $fa2, $fa1
	move	$a0, $s1
	bstrins.d	$a0, $s2, 63, 32
	movgr2fr.d	$fa2, $a0
	fadd.d	$fa1, $fa2, $fa1
	fdiv.d	$fa0, $fa0, $fa1
	movfr2gr.d	$a1, $fa0
	pcalau12i	$a0, %pc_hi20(.L.str.8)
	addi.d	$a0, $a0, %pc_lo12(.L.str.8)
	pcaddu18i	$ra, %call36(printf)
	jirl	$ra, $ra, 0
	bstrins.d	$s0, $zero, 0, 0
	ori	$a0, $zero, 2
	bne	$s0, $a0, .LBB3_27
.LBB3_29:
	pcalau12i	$a0, %pc_hi20(prog_time)
	ld.d	$a0, $a0, %pc_lo12(prog_time)
	srli.d	$a1, $a0, 32
	or	$a1, $a1, $s3
	movgr2fr.d	$fa0, $a1
	lu52i.d	$a1, $s5, 1107
	movgr2fr.d	$fa1, $a1
	add.d	$a1, $a0, $s1
	bstrins.d	$a0, $s2, 63, 32
	ld.d	$a2, $s4, %pc_lo12(loop_time)
	movgr2fr.d	$fa2, $a0
	fsub.d	$fa0, $fa0, $fa1
	fadd.d	$fa0, $fa2, $fa0
	sub.d	$a0, $a1, $a2
	srli.d	$a1, $a0, 32
	or	$a1, $a1, $s3
	movgr2fr.d	$fa2, $a1
	fsub.d	$fa1, $fa2, $fa1
	bstrins.d	$a0, $s2, 63, 32
	movgr2fr.d	$fa2, $a0
	fadd.d	$fa1, $fa2, $fa1
	fdiv.d	$fa0, $fa0, $fa1
	movfr2gr.d	$a1, $fa0
	pcalau12i	$a0, %pc_hi20(.L.str.9)
	addi.d	$a0, $a0, %pc_lo12(.L.str.9)
	pcaddu18i	$ra, %call36(printf)
	jirl	$ra, $ra, 0
	beqz	$fp, .LBB3_34
.LBB3_30:
	ori	$a0, $zero, 1
	bne	$s6, $a0, .LBB3_32
# %bb.31:
	ld.d	$a0, $s4, %pc_lo12(loop_time)
	srli.d	$a1, $a0, 32
	or	$a1, $a1, $s3
	movgr2fr.d	$fa0, $a1
	lu52i.d	$a1, $s5, 1107
	movgr2fr.d	$fa1, $a1
	fsub.d	$fa0, $fa0, $fa1
	bstrins.d	$a0, $s2, 63, 32
	movgr2fr.d	$fa2, $a0
	fadd.d	$fa0, $fa2, $fa0
	srli.d	$a0, $s1, 32
	or	$a0, $a0, $s3
	movgr2fr.d	$fa2, $a0
	fsub.d	$fa1, $fa2, $fa1
	move	$a0, $s1
	bstrins.d	$a0, $s2, 63, 32
	movgr2fr.d	$fa2, $a0
	fadd.d	$fa1, $fa2, $fa1
	fdiv.d	$fa0, $fa0, $fa1
	movfr2gr.d	$a2, $fa0
	pcalau12i	$a0, %pc_hi20(.L.str.10)
	addi.d	$a1, $a0, %pc_lo12(.L.str.10)
	move	$a0, $fp
	pcaddu18i	$ra, %call36(fprintf)
	jirl	$ra, $ra, 0
.LBB3_32:
	ori	$a0, $zero, 2
	bne	$s0, $a0, .LBB3_34
# %bb.33:
	pcalau12i	$a0, %pc_hi20(prog_time)
	ld.d	$a0, $a0, %pc_lo12(prog_time)
	srli.d	$a1, $a0, 32
	or	$a1, $a1, $s3
	movgr2fr.d	$fa0, $a1
	lu52i.d	$a1, $s5, 1107
	movgr2fr.d	$fa1, $a1
	fsub.d	$fa0, $fa0, $fa1
	add.d	$a1, $a0, $s1
	ld.d	$a2, $s4, %pc_lo12(loop_time)
	bstrins.d	$a0, $s2, 63, 32
	movgr2fr.d	$fa2, $a0
	fadd.d	$fa0, $fa2, $fa0
	sub.d	$a0, $a1, $a2
	srli.d	$a1, $a0, 32
	or	$a1, $a1, $s3
	movgr2fr.d	$fa2, $a1
	fsub.d	$fa1, $fa2, $fa1
	bstrins.d	$a0, $s2, 63, 32
	movgr2fr.d	$fa2, $a0
	fadd.d	$fa1, $fa2, $fa1
	fdiv.d	$fa0, $fa0, $fa1
	movfr2gr.d	$a2, $fa0
	pcalau12i	$a0, %pc_hi20(.L.str.11)
	addi.d	$a1, $a0, %pc_lo12(.L.str.11)
	move	$a0, $fp
	ld.d	$s6, $sp, 8                     # 8-byte Folded Reload
	ld.d	$s5, $sp, 16                    # 8-byte Folded Reload
	ld.d	$s4, $sp, 24                    # 8-byte Folded Reload
	ld.d	$s3, $sp, 32                    # 8-byte Folded Reload
	ld.d	$s2, $sp, 40                    # 8-byte Folded Reload
	ld.d	$s1, $sp, 48                    # 8-byte Folded Reload
	ld.d	$s0, $sp, 56                    # 8-byte Folded Reload
	ld.d	$fp, $sp, 64                    # 8-byte Folded Reload
	ld.d	$ra, $sp, 72                    # 8-byte Folded Reload
	addi.d	$sp, $sp, 80
	pcaddu18i	$t8, %call36(fprintf)
	jr	$t8
.LBB3_34:
	ld.d	$s6, $sp, 8                     # 8-byte Folded Reload
	ld.d	$s5, $sp, 16                    # 8-byte Folded Reload
	ld.d	$s4, $sp, 24                    # 8-byte Folded Reload
	ld.d	$s3, $sp, 32                    # 8-byte Folded Reload
	ld.d	$s2, $sp, 40                    # 8-byte Folded Reload
	ld.d	$s1, $sp, 48                    # 8-byte Folded Reload
	ld.d	$s0, $sp, 56                    # 8-byte Folded Reload
	ld.d	$fp, $sp, 64                    # 8-byte Folded Reload
	ld.d	$ra, $sp, 72                    # 8-byte Folded Reload
	addi.d	$sp, $sp, 80
	ret
.Lfunc_end3:
	.size	specul_time_o, .Lfunc_end3-specul_time_o
                                        # -- End function
	.globl	specul_time_r                   # -- Begin function specul_time_r
	.p2align	5
	.type	specul_time_r,@function
specul_time_r:                          # @specul_time_r
# %bb.0:
	addi.d	$sp, $sp, -128
	st.d	$ra, $sp, 120                   # 8-byte Folded Spill
	st.d	$fp, $sp, 112                   # 8-byte Folded Spill
	st.d	$s0, $sp, 104                   # 8-byte Folded Spill
	st.d	$s1, $sp, 96                    # 8-byte Folded Spill
	st.d	$s2, $sp, 88                    # 8-byte Folded Spill
	st.d	$s3, $sp, 80                    # 8-byte Folded Spill
	st.d	$s4, $sp, 72                    # 8-byte Folded Spill
	st.d	$s5, $sp, 64                    # 8-byte Folded Spill
	st.d	$s6, $sp, 56                    # 8-byte Folded Spill
	st.d	$s7, $sp, 48                    # 8-byte Folded Spill
	st.d	$s8, $sp, 40                    # 8-byte Folded Spill
	st.d	$a6, $sp, 16                    # 8-byte Folded Spill
	st.d	$a5, $sp, 8                     # 8-byte Folded Spill
	st.d	$a4, $sp, 24                    # 8-byte Folded Spill
	st.d	$a3, $sp, 32                    # 8-byte Folded Spill
	move	$fp, $a2
	move	$s4, $a1
	move	$s5, $a0
	beqz	$a2, .LBB4_27
# %bb.1:
	pcalau12i	$a0, %pc_hi20(.L.str.13)
	addi.d	$a0, $a0, %pc_lo12(.L.str.13)
	move	$a1, $fp
	pcaddu18i	$ra, %call36(printf)
	jirl	$ra, $ra, 0
	addi.d	$a0, $s5, 48
	blez	$s4, .LBB4_4
# %bb.2:                                # %.lr.ph.preheader
	move	$a1, $s4
	move	$a2, $a0
	.p2align	4, , 16
.LBB4_3:                                # %.lr.ph
                                        # =>This Inner Loop Header: Depth=1
	ld.d	$a3, $a2, -16
	ld.d	$a4, $a2, -24
	sub.d	$a3, $a3, $a4
	st.d	$a3, $a2, -8
	st.d	$zero, $a2, 0
	addi.d	$a1, $a1, -1
	addi.d	$a2, $a2, 56
	bnez	$a1, .LBB4_3
.LBB4_4:                                # %.preheader181
	bge	$fp, $s4, .LBB4_22
# %bb.5:                                # %.lr.ph190.preheader
	move	$a1, $zero
	move	$a2, $zero
	addi.d	$a3, $fp, 1
	addi.d	$a4, $s5, 104
	slli.d	$a5, $fp, 1
	sub.d	$a6, $zero, $fp
	ori	$a7, $zero, 56
	mul.d	$t0, $fp, $a7
	add.d	$t1, $a4, $t0
	addi.w	$t2, $zero, -2
	ori	$t3, $zero, 2
	move	$t4, $a3
	move	$t5, $fp
	b	.LBB4_8
	.p2align	4, , 16
.LBB4_6:                                # %.preheader179
                                        #   in Loop: Header=BB4_8 Depth=1
	move	$t8, $t5
.LBB4_7:                                # %.loopexit180
                                        #   in Loop: Header=BB4_8 Depth=1
	add.w	$a2, $a2, $fp
	addi.d	$a1, $a1, 1
	add.d	$a5, $a5, $fp
	add.d	$t4, $t4, $fp
	sub.d	$a6, $a6, $fp
	add.d	$t1, $t1, $t0
	bge	$t8, $s4, .LBB4_22
.LBB4_8:                                # %.lr.ph190
                                        # =>This Loop Header: Depth=1
                                        #     Child Loop BB4_12 Depth 2
                                        #     Child Loop BB4_15 Depth 2
                                        #     Child Loop BB4_18 Depth 2
                                        #     Child Loop BB4_21 Depth 2
	move	$t6, $t5
	add.d	$t5, $t5, $fp
	blez	$fp, .LBB4_6
# %bb.9:                                # %.lr.ph185.preheader
                                        #   in Loop: Header=BB4_8 Depth=1
	addi.d	$t7, $a2, 1
	slt	$t8, $t7, $t6
	masknez	$t7, $t7, $t8
	maskeqz	$t8, $t6, $t8
	or	$t7, $t8, $t7
	sub.d	$s6, $t7, $a2
	bgeu	$s6, $t3, .LBB4_11
# %bb.10:                               #   in Loop: Header=BB4_8 Depth=1
	move	$t7, $zero
	move	$t8, $a2
	b	.LBB4_14
	.p2align	4, , 16
.LBB4_11:                               # %vector.ph266
                                        #   in Loop: Header=BB4_8 Depth=1
	move	$t7, $zero
	move	$s8, $zero
	and	$ra, $s6, $t2
	mul.d	$t8, $a2, $a7
	addi.d	$a4, $s5, 104
	add.d	$s2, $a4, $t8
	move	$s7, $s6
	bstrins.d	$s7, $zero, 0, 0
	add.d	$t8, $s7, $a2
	.p2align	4, , 16
.LBB4_12:                               # %vector.body269
                                        #   Parent Loop BB4_8 Depth=1
                                        # =>  This Inner Loop Header: Depth=2
	ld.d	$s3, $s2, -64
	ld.d	$s0, $s2, -8
	ld.d	$s1, $s2, -56
	ld.d	$a4, $s2, 0
	add.d	$s1, $s1, $s3
	add.d	$a4, $a4, $s0
	sltu	$s0, $t7, $s1
	maskeqz	$s1, $s1, $s0
	masknez	$t7, $t7, $s0
	or	$t7, $s1, $t7
	sltu	$s0, $s8, $a4
	maskeqz	$a4, $a4, $s0
	masknez	$s0, $s8, $s0
	or	$s8, $a4, $s0
	addi.d	$ra, $ra, -2
	addi.d	$s2, $s2, 112
	bnez	$ra, .LBB4_12
# %bb.13:                               # %middle.block274
                                        #   in Loop: Header=BB4_8 Depth=1
	sltu	$a4, $s8, $t7
	masknez	$s0, $s8, $a4
	maskeqz	$a4, $t7, $a4
	or	$t7, $a4, $s0
	beq	$s6, $s7, .LBB4_16
.LBB4_14:                               # %.lr.ph185.preheader339
                                        #   in Loop: Header=BB4_8 Depth=1
	mul.d	$a4, $t8, $a7
	add.d	$s2, $a0, $a4
	.p2align	4, , 16
.LBB4_15:                               # %.lr.ph185
                                        #   Parent Loop BB4_8 Depth=1
                                        # =>  This Inner Loop Header: Depth=2
	ld.d	$a4, $s2, -8
	ld.d	$s0, $s2, 0
	add.d	$a4, $s0, $a4
	sltu	$s0, $t7, $a4
	maskeqz	$a4, $a4, $s0
	masknez	$t7, $t7, $s0
	or	$t7, $a4, $t7
	addi.d	$t8, $t8, 1
	addi.d	$s2, $s2, 56
	blt	$t8, $t6, .LBB4_15
.LBB4_16:                               # %.lr.ph187.preheader
                                        #   in Loop: Header=BB4_8 Depth=1
	mul.d	$a4, $a1, $fp
	addi.d	$t8, $a1, 2
	mul.d	$t8, $t8, $fp
	add.d	$a4, $a3, $a4
	slt	$s0, $a4, $t8
	masknez	$a4, $a4, $s0
	maskeqz	$t8, $t8, $s0
	or	$a4, $t8, $a4
	nor	$t8, $a1, $zero
	mul.d	$t8, $t8, $fp
	add.d	$s2, $a4, $t8
	add.d	$t8, $t6, $fp
	bltu	$s2, $t3, .LBB4_20
# %bb.17:                               # %vector.ph
                                        #   in Loop: Header=BB4_8 Depth=1
	slt	$a4, $t4, $a5
	masknez	$s0, $t4, $a4
	maskeqz	$a4, $a5, $a4
	or	$a4, $a4, $s0
	add.d	$a4, $a4, $a6
	and	$s7, $a4, $t2
	move	$s6, $s2
	bstrins.d	$s6, $zero, 0, 0
	add.d	$t6, $t6, $s6
	move	$s8, $t1
	.p2align	4, , 16
.LBB4_18:                               # %vector.body
                                        #   Parent Loop BB4_8 Depth=1
                                        # =>  This Inner Loop Header: Depth=2
	st.d	$t7, $s8, -56
	st.d	$t7, $s8, 0
	addi.d	$s7, $s7, -2
	addi.d	$s8, $s8, 112
	bnez	$s7, .LBB4_18
# %bb.19:                               # %middle.block
                                        #   in Loop: Header=BB4_8 Depth=1
	beq	$s2, $s6, .LBB4_7
.LBB4_20:                               # %.lr.ph187.preheader338
                                        #   in Loop: Header=BB4_8 Depth=1
	mul.d	$a4, $t6, $a7
	add.d	$s2, $a0, $a4
	.p2align	4, , 16
.LBB4_21:                               # %.lr.ph187
                                        #   Parent Loop BB4_8 Depth=1
                                        # =>  This Inner Loop Header: Depth=2
	st.d	$t7, $s2, 0
	addi.d	$t6, $t6, 1
	addi.d	$s2, $s2, 56
	blt	$t6, $t8, .LBB4_21
	b	.LBB4_7
.LBB4_22:                               # %.preheader178
	pcalau12i	$a0, %got_pc_hi20(hard_raw_list)
	ld.d	$a0, $a0, %got_pc_lo12(hard_raw_list)
	ld.d	$a1, $a0, 0
	beqz	$a1, .LBB4_35
# %bb.23:                               # %.lr.ph195.preheader
	move	$a0, $zero
	ori	$a2, $zero, 56
	b	.LBB4_25
	.p2align	4, , 16
.LBB4_24:                               #   in Loop: Header=BB4_25 Depth=1
	ld.d	$a1, $a1, 0
	beqz	$a1, .LBB4_36
.LBB4_25:                               # %.lr.ph195
                                        # =>This Inner Loop Header: Depth=1
	ld.w	$a3, $a1, 24
	ld.d	$a7, $a1, 32
	ld.w	$a5, $a1, 40
	mul.d	$a3, $a3, $a2
	add.d	$a3, $s5, $a3
	ld.d	$a4, $a3, 48
	mul.d	$a5, $a5, $a2
	add.d	$a5, $s5, $a5
	ld.d	$a6, $a5, 48
	add.d	$a7, $a4, $a7
	bgeu	$a6, $a7, .LBB4_24
# %bb.26:                               #   in Loop: Header=BB4_25 Depth=1
	ld.d	$a3, $a3, 40
	add.d	$a4, $a6, $a4
	add.d	$a3, $a4, $a3
	st.d	$a3, $a5, 48
	addi.d	$a0, $a0, 1
	b	.LBB4_24
.LBB4_27:
	pcalau12i	$a0, %pc_hi20(.Lstr.1)
	addi.d	$a0, $a0, %pc_lo12(.Lstr.1)
	pcaddu18i	$ra, %call36(puts)
	jirl	$ra, $ra, 0
	blez	$s4, .LBB4_30
# %bb.28:                               # %.lr.ph206.preheader
	addi.d	$a0, $s5, 48
	move	$a1, $s4
	.p2align	4, , 16
.LBB4_29:                               # %.lr.ph206
                                        # =>This Inner Loop Header: Depth=1
	ld.d	$a2, $a0, -16
	ld.d	$a3, $a0, -24
	sub.d	$a2, $a2, $a3
	st.d	$a2, $a0, -8
	st.d	$zero, $a0, 0
	addi.d	$a1, $a1, -1
	addi.d	$a0, $a0, 56
	bnez	$a1, .LBB4_29
.LBB4_30:                               # %.preheader175
	pcalau12i	$a0, %got_pc_hi20(hard_raw_list)
	ld.d	$a0, $a0, %got_pc_lo12(hard_raw_list)
	ld.d	$a1, $a0, 0
	beqz	$a1, .LBB4_51
# %bb.31:                               # %.lr.ph211.preheader
	move	$a0, $zero
	ori	$a2, $zero, 56
	b	.LBB4_33
	.p2align	4, , 16
.LBB4_32:                               #   in Loop: Header=BB4_33 Depth=1
	ld.d	$a1, $a1, 0
	beqz	$a1, .LBB4_52
.LBB4_33:                               # %.lr.ph211
                                        # =>This Inner Loop Header: Depth=1
	ld.w	$a3, $a1, 24
	ld.d	$a6, $a1, 32
	ld.w	$a5, $a1, 40
	mul.d	$a3, $a3, $a2
	add.d	$a3, $s5, $a3
	ld.d	$a4, $a3, 48
	mul.d	$a5, $a5, $a2
	add.d	$a5, $s5, $a5
	ld.d	$a7, $a5, 48
	add.d	$a6, $a4, $a6
	bgeu	$a7, $a6, .LBB4_32
# %bb.34:                               #   in Loop: Header=BB4_33 Depth=1
	ld.d	$a3, $a3, 40
	add.d	$a3, $a3, $a4
	st.d	$a3, $a5, 48
	addi.d	$a0, $a0, 1
	b	.LBB4_32
.LBB4_35:
	move	$a0, $zero
.LBB4_36:                               # %.preheader177
	blez	$s4, .LBB4_55
# %bb.37:                               # %.lr.ph198.preheader
	ori	$a1, $zero, 1
	bstrpick.d	$a2, $s4, 30, 1
	bne	$s4, $a1, .LBB4_39
# %bb.38:
	move	$a1, $zero
	b	.LBB4_42
.LBB4_39:                               # %vector.ph279
	slli.d	$a1, $a2, 1
	addi.d	$a3, $s5, 96
	move	$a4, $a1
	.p2align	4, , 16
.LBB4_40:                               # %vector.body282
                                        # =>This Inner Loop Header: Depth=1
	ld.d	$a5, $a3, -56
	ld.d	$a6, $a3, 0
	ld.d	$a7, $a3, -48
	ld.d	$t0, $a3, 8
	add.d	$a5, $a7, $a5
	add.d	$a6, $t0, $a6
	st.d	$a5, $a3, -56
	st.d	$a6, $a3, 0
	addi.d	$a4, $a4, -2
	addi.d	$a3, $a3, 112
	bnez	$a4, .LBB4_40
# %bb.41:                               # %middle.block285
	beq	$a1, $s4, .LBB4_44
.LBB4_42:                               # %.lr.ph198.preheader337
	ori	$a3, $zero, 56
	mul.d	$a3, $a1, $a3
	add.d	$a3, $a3, $s5
	addi.d	$a3, $a3, 40
	sub.d	$a1, $s4, $a1
	.p2align	4, , 16
.LBB4_43:                               # %.lr.ph198
                                        # =>This Inner Loop Header: Depth=1
	ld.d	$a4, $a3, 0
	ld.d	$a5, $a3, 8
	add.d	$a4, $a5, $a4
	st.d	$a4, $a3, 0
	addi.d	$a1, $a1, -1
	addi.d	$a3, $a3, 56
	bnez	$a1, .LBB4_43
.LBB4_44:                               # %.lr.ph202.preheader
	ld.d	$a1, $s5, 40
	ori	$a3, $zero, 1
	bne	$s4, $a3, .LBB4_46
# %bb.45:
	move	$a2, $zero
	b	.LBB4_49
.LBB4_46:                               # %vector.ph290
	slli.d	$a2, $a2, 1
	addi.d	$a3, $s5, 96
	move	$a4, $a2
	move	$a5, $a1
	.p2align	4, , 16
.LBB4_47:                               # %vector.body293
                                        # =>This Inner Loop Header: Depth=1
	ld.d	$a6, $a3, -56
	ld.d	$a7, $a3, 0
	sltu	$t0, $a1, $a6
	masknez	$a1, $a1, $t0
	maskeqz	$a6, $a6, $t0
	or	$a1, $a6, $a1
	sltu	$a6, $a5, $a7
	masknez	$a5, $a5, $a6
	maskeqz	$a6, $a7, $a6
	or	$a5, $a6, $a5
	addi.d	$a4, $a4, -2
	addi.d	$a3, $a3, 112
	bnez	$a4, .LBB4_47
# %bb.48:                               # %middle.block298
	sltu	$a3, $a5, $a1
	masknez	$a4, $a5, $a3
	maskeqz	$a1, $a1, $a3
	or	$a1, $a1, $a4
	beq	$a2, $s4, .LBB4_68
.LBB4_49:                               # %.lr.ph202.preheader333
	ori	$a3, $zero, 56
	mul.d	$a3, $a2, $a3
	add.d	$a3, $a3, $s5
	addi.d	$a3, $a3, 40
	sub.d	$a2, $s4, $a2
	.p2align	4, , 16
.LBB4_50:                               # %.lr.ph202
                                        # =>This Inner Loop Header: Depth=1
	ld.d	$a4, $a3, 0
	sltu	$a5, $a1, $a4
	masknez	$a1, $a1, $a5
	maskeqz	$a4, $a4, $a5
	or	$a1, $a4, $a1
	addi.d	$a2, $a2, -1
	addi.d	$a3, $a3, 56
	bnez	$a2, .LBB4_50
	b	.LBB4_68
.LBB4_51:
	move	$a0, $zero
.LBB4_52:                               # %.preheader
	blez	$s4, .LBB4_55
# %bb.53:                               # %.lr.ph214.preheader
	ori	$a1, $zero, 1
	bne	$s4, $a1, .LBB4_56
# %bb.54:
	move	$a1, $zero
	b	.LBB4_59
.LBB4_55:
	ld.d	$a2, $sp, 16                    # 8-byte Folded Reload
	ld.d	$a1, $s5, 40
	b	.LBB4_69
.LBB4_56:                               # %vector.ph305
	bstrpick.d	$a1, $s4, 30, 1
	slli.d	$a1, $a1, 1
	addi.d	$a2, $s5, 96
	move	$a3, $a1
	.p2align	4, , 16
.LBB4_57:                               # %vector.body308
                                        # =>This Inner Loop Header: Depth=1
	ld.d	$a4, $a2, -56
	ld.d	$a5, $a2, 0
	ld.d	$a6, $a2, -48
	ld.d	$a7, $a2, 8
	add.d	$a4, $a6, $a4
	add.d	$a5, $a7, $a5
	st.d	$a4, $a2, -56
	st.d	$a5, $a2, 0
	addi.d	$a3, $a3, -2
	addi.d	$a2, $a2, 112
	bnez	$a3, .LBB4_57
# %bb.58:                               # %middle.block311
	beq	$a1, $s4, .LBB4_61
.LBB4_59:                               # %.lr.ph214.preheader332
	ori	$a2, $zero, 56
	mul.d	$a2, $a1, $a2
	add.d	$a2, $a2, $s5
	addi.d	$a2, $a2, 40
	sub.d	$a1, $s4, $a1
	.p2align	4, , 16
.LBB4_60:                               # %.lr.ph214
                                        # =>This Inner Loop Header: Depth=1
	ld.d	$a3, $a2, 0
	ld.d	$a4, $a2, 8
	add.d	$a3, $a4, $a3
	st.d	$a3, $a2, 0
	addi.d	$a1, $a1, -1
	addi.d	$a2, $a2, 56
	bnez	$a1, .LBB4_60
.LBB4_61:                               # %.lr.ph219.preheader
	ld.d	$a1, $s5, 40
	ori	$a2, $zero, 1
	bne	$s4, $a2, .LBB4_63
# %bb.62:
	move	$a2, $zero
	b	.LBB4_66
.LBB4_63:                               # %vector.ph316
	bstrpick.d	$a2, $s4, 30, 1
	slli.d	$a2, $a2, 1
	addi.d	$a3, $s5, 96
	move	$a4, $a2
	move	$a5, $a1
	.p2align	4, , 16
.LBB4_64:                               # %vector.body319
                                        # =>This Inner Loop Header: Depth=1
	ld.d	$a6, $a3, -56
	ld.d	$a7, $a3, 0
	sltu	$t0, $a1, $a6
	masknez	$a1, $a1, $t0
	maskeqz	$a6, $a6, $t0
	or	$a1, $a6, $a1
	sltu	$a6, $a5, $a7
	masknez	$a5, $a5, $a6
	maskeqz	$a6, $a7, $a6
	or	$a5, $a6, $a5
	addi.d	$a4, $a4, -2
	addi.d	$a3, $a3, 112
	bnez	$a4, .LBB4_64
# %bb.65:                               # %middle.block324
	sltu	$a3, $a5, $a1
	masknez	$a4, $a5, $a3
	maskeqz	$a1, $a1, $a3
	or	$a1, $a1, $a4
	beq	$a2, $s4, .LBB4_68
.LBB4_66:                               # %.lr.ph219.preheader330
	ori	$a3, $zero, 56
	mul.d	$a3, $a2, $a3
	add.d	$a3, $a3, $s5
	addi.d	$a3, $a3, 40
	sub.d	$a2, $s4, $a2
	.p2align	4, , 16
.LBB4_67:                               # %.lr.ph219
                                        # =>This Inner Loop Header: Depth=1
	ld.d	$a4, $a3, 0
	sltu	$a5, $a1, $a4
	masknez	$a1, $a1, $a5
	maskeqz	$a4, $a4, $a5
	or	$a1, $a4, $a1
	addi.d	$a2, $a2, -1
	addi.d	$a3, $a3, 56
	bnez	$a2, .LBB4_67
.LBB4_68:
	ld.d	$a2, $sp, 16                    # 8-byte Folded Reload
.LBB4_69:                               # %.loopexit
	ld.d	$a3, $sp, 8                     # 8-byte Folded Reload
	add.d	$a0, $a0, $s4
	mul.d	$a0, $a0, $a3
	mul.w	$a2, $a2, $s4
	add.d	$a1, $a1, $a2
	add.d	$s2, $a1, $a0
	pcalau12i	$a0, %pc_hi20(.L.str.7)
	addi.d	$a0, $a0, %pc_lo12(.L.str.7)
	move	$a1, $s2
	pcaddu18i	$ra, %call36(printf)
	jirl	$ra, $ra, 0
	ld.d	$a4, $sp, 24                    # 8-byte Folded Reload
	move	$s7, $a4
	bstrins.d	$s7, $zero, 1, 1
	ori	$a0, $zero, 1
	pcalau12i	$s5, %pc_hi20(loop_time)
	lu52i.d	$s4, $zero, 1107
	lu12i.w	$s6, 256
	lu12i.w	$s3, 275200
	beq	$s7, $a0, .LBB4_72
# %bb.70:
	bstrins.d	$a4, $zero, 0, 0
	ori	$a0, $zero, 2
	beq	$a4, $a0, .LBB4_73
.LBB4_71:
	ld.d	$a0, $sp, 32                    # 8-byte Folded Reload
	bnez	$a0, .LBB4_74
	b	.LBB4_78
.LBB4_72:
	ld.d	$a0, $s5, %pc_lo12(loop_time)
	srli.d	$a1, $a0, 32
	or	$a1, $a1, $s4
	movgr2fr.d	$fa0, $a1
	lu52i.d	$a1, $s6, 1107
	movgr2fr.d	$fa1, $a1
	fsub.d	$fa0, $fa0, $fa1
	bstrins.d	$a0, $s3, 63, 32
	movgr2fr.d	$fa2, $a0
	fadd.d	$fa0, $fa2, $fa0
	srli.d	$a0, $s2, 32
	or	$a0, $a0, $s4
	movgr2fr.d	$fa2, $a0
	fsub.d	$fa1, $fa2, $fa1
	move	$a0, $s2
	bstrins.d	$a0, $s3, 63, 32
	movgr2fr.d	$fa2, $a0
	fadd.d	$fa1, $fa2, $fa1
	fdiv.d	$fa0, $fa0, $fa1
	movfr2gr.d	$a1, $fa0
	pcalau12i	$a0, %pc_hi20(.L.str.8)
	addi.d	$a0, $a0, %pc_lo12(.L.str.8)
	pcaddu18i	$ra, %call36(printf)
	jirl	$ra, $ra, 0
	ld.d	$a4, $sp, 24                    # 8-byte Folded Reload
	bstrins.d	$a4, $zero, 0, 0
	ori	$a0, $zero, 2
	bne	$a4, $a0, .LBB4_71
.LBB4_73:
	pcalau12i	$a0, %pc_hi20(prog_time)
	ld.d	$a0, $a0, %pc_lo12(prog_time)
	srli.d	$a1, $a0, 32
	or	$a1, $a1, $s4
	movgr2fr.d	$fa0, $a1
	lu52i.d	$a1, $s6, 1107
	movgr2fr.d	$fa1, $a1
	add.d	$a1, $a0, $s2
	bstrins.d	$a0, $s3, 63, 32
	ld.d	$a2, $s5, %pc_lo12(loop_time)
	movgr2fr.d	$fa2, $a0
	fsub.d	$fa0, $fa0, $fa1
	fadd.d	$fa0, $fa2, $fa0
	sub.d	$a0, $a1, $a2
	srli.d	$a1, $a0, 32
	or	$a1, $a1, $s4
	movgr2fr.d	$fa2, $a1
	fsub.d	$fa1, $fa2, $fa1
	bstrins.d	$a0, $s3, 63, 32
	movgr2fr.d	$fa2, $a0
	fadd.d	$fa1, $fa2, $fa1
	fdiv.d	$fa0, $fa0, $fa1
	movfr2gr.d	$a1, $fa0
	pcalau12i	$a0, %pc_hi20(.L.str.9)
	addi.d	$a0, $a0, %pc_lo12(.L.str.9)
	move	$s0, $a4
	pcaddu18i	$ra, %call36(printf)
	jirl	$ra, $ra, 0
	move	$a4, $s0
	ld.d	$a0, $sp, 32                    # 8-byte Folded Reload
	beqz	$a0, .LBB4_78
.LBB4_74:
	ori	$a0, $zero, 1
	bne	$s7, $a0, .LBB4_76
# %bb.75:
	ld.d	$a0, $s5, %pc_lo12(loop_time)
	srli.d	$a1, $a0, 32
	or	$a1, $a1, $s4
	movgr2fr.d	$fa0, $a1
	lu52i.d	$a1, $s6, 1107
	movgr2fr.d	$fa1, $a1
	fsub.d	$fa0, $fa0, $fa1
	bstrins.d	$a0, $s3, 63, 32
	movgr2fr.d	$fa2, $a0
	fadd.d	$fa0, $fa2, $fa0
	srli.d	$a0, $s2, 32
	or	$a0, $a0, $s4
	movgr2fr.d	$fa2, $a0
	fsub.d	$fa1, $fa2, $fa1
	move	$a0, $s2
	bstrins.d	$a0, $s3, 63, 32
	movgr2fr.d	$fa2, $a0
	fadd.d	$fa1, $fa2, $fa1
	fdiv.d	$fa0, $fa0, $fa1
	movfr2gr.d	$a3, $fa0
	pcalau12i	$a0, %pc_hi20(.L.str.14)
	addi.d	$a1, $a0, %pc_lo12(.L.str.14)
	ld.d	$a0, $sp, 32                    # 8-byte Folded Reload
	move	$a2, $fp
	move	$s0, $a4
	pcaddu18i	$ra, %call36(fprintf)
	jirl	$ra, $ra, 0
	move	$a4, $s0
.LBB4_76:
	ori	$a0, $zero, 2
	bne	$a4, $a0, .LBB4_78
# %bb.77:
	pcalau12i	$a0, %pc_hi20(prog_time)
	ld.d	$a0, $a0, %pc_lo12(prog_time)
	srli.d	$a1, $a0, 32
	or	$a1, $a1, $s4
	movgr2fr.d	$fa0, $a1
	lu52i.d	$a1, $s6, 1107
	movgr2fr.d	$fa1, $a1
	fsub.d	$fa0, $fa0, $fa1
	add.d	$a1, $a0, $s2
	ld.d	$a2, $s5, %pc_lo12(loop_time)
	bstrins.d	$a0, $s3, 63, 32
	movgr2fr.d	$fa2, $a0
	fadd.d	$fa0, $fa2, $fa0
	sub.d	$a0, $a1, $a2
	srli.d	$a1, $a0, 32
	or	$a1, $a1, $s4
	movgr2fr.d	$fa2, $a1
	fsub.d	$fa1, $fa2, $fa1
	bstrins.d	$a0, $s3, 63, 32
	movgr2fr.d	$fa2, $a0
	fadd.d	$fa1, $fa2, $fa1
	fdiv.d	$fa0, $fa0, $fa1
	movfr2gr.d	$a3, $fa0
	pcalau12i	$a0, %pc_hi20(.L.str.15)
	addi.d	$a1, $a0, %pc_lo12(.L.str.15)
	ld.d	$a0, $sp, 32                    # 8-byte Folded Reload
	move	$a2, $fp
	ld.d	$s8, $sp, 40                    # 8-byte Folded Reload
	ld.d	$s7, $sp, 48                    # 8-byte Folded Reload
	ld.d	$s6, $sp, 56                    # 8-byte Folded Reload
	ld.d	$s5, $sp, 64                    # 8-byte Folded Reload
	ld.d	$s4, $sp, 72                    # 8-byte Folded Reload
	ld.d	$s3, $sp, 80                    # 8-byte Folded Reload
	ld.d	$s2, $sp, 88                    # 8-byte Folded Reload
	ld.d	$s1, $sp, 96                    # 8-byte Folded Reload
	ld.d	$s0, $sp, 104                   # 8-byte Folded Reload
	ld.d	$fp, $sp, 112                   # 8-byte Folded Reload
	ld.d	$ra, $sp, 120                   # 8-byte Folded Reload
	addi.d	$sp, $sp, 128
	pcaddu18i	$t8, %call36(fprintf)
	jr	$t8
.LBB4_78:
	ld.d	$s8, $sp, 40                    # 8-byte Folded Reload
	ld.d	$s7, $sp, 48                    # 8-byte Folded Reload
	ld.d	$s6, $sp, 56                    # 8-byte Folded Reload
	ld.d	$s5, $sp, 64                    # 8-byte Folded Reload
	ld.d	$s4, $sp, 72                    # 8-byte Folded Reload
	ld.d	$s3, $sp, 80                    # 8-byte Folded Reload
	ld.d	$s2, $sp, 88                    # 8-byte Folded Reload
	ld.d	$s1, $sp, 96                    # 8-byte Folded Reload
	ld.d	$s0, $sp, 104                   # 8-byte Folded Reload
	ld.d	$fp, $sp, 112                   # 8-byte Folded Reload
	ld.d	$ra, $sp, 120                   # 8-byte Folded Reload
	addi.d	$sp, $sp, 128
	ret
.Lfunc_end4:
	.size	specul_time_r, .Lfunc_end4-specul_time_r
                                        # -- End function
	.type	.L.str,@object                  # @.str
	.section	.rodata.str1.1,"aMS",@progbits,1
.L.str:
	.asciz	"%s %lu"
	.size	.L.str, 7

	.type	.L.str.1,@object                # @.str.1
.L.str.1:
	.asciz	"%s %lx %lu"
	.size	.L.str.1, 11

	.type	.L.str.2,@object                # @.str.2
.L.str.2:
	.asciz	"%s %lu %lu"
	.size	.L.str.2, 11

	.type	loop_time,@object               # @loop_time
	.bss
	.globl	loop_time
	.p2align	3, 0x0
loop_time:
	.dword	0                               # 0x0
	.size	loop_time, 8

	.type	.L.str.3,@object                # @.str.3
	.section	.rodata.str1.1,"aMS",@progbits,1
.L.str.3:
	.asciz	"Time for loop: %lu issues\n"
	.size	.L.str.3, 27

	.type	prog_time,@object               # @prog_time
	.bss
	.globl	prog_time
	.p2align	3, 0x0
prog_time:
	.dword	0                               # 0x0
	.size	prog_time, 8

	.type	.L.str.4,@object                # @.str.4
	.section	.rodata.str1.1,"aMS",@progbits,1
.L.str.4:
	.asciz	"Time for program: %lu issues\n"
	.size	.L.str.4, 30

	.type	.L.str.5,@object                # @.str.5
.L.str.5:
	.asciz	"Loop is %0.3g %% of program\n"
	.size	.L.str.5, 29

	.type	.L.str.7,@object                # @.str.7
.L.str.7:
	.asciz	"Time for speculative loop is %lu issues\n"
	.size	.L.str.7, 41

	.type	.L.str.8,@object                # @.str.8
.L.str.8:
	.asciz	"Potential speedup for loop: %0.3g times\n"
	.size	.L.str.8, 41

	.type	.L.str.9,@object                # @.str.9
.L.str.9:
	.asciz	"Potential speedup for program: %0.3g times\n"
	.size	.L.str.9, 44

	.type	.L.str.10,@object               # @.str.10
.L.str.10:
	.asciz	"optloop: %0.3g\n"
	.size	.L.str.10, 16

	.type	.L.str.11,@object               # @.str.11
.L.str.11:
	.asciz	"optprog: %0.3g\n"
	.size	.L.str.11, 16

	.type	.L.str.13,@object               # @.str.13
.L.str.13:
	.asciz	"REALISTIC RESTART RESULTS -- %d CPUs\n"
	.size	.L.str.13, 38

	.type	.L.str.14,@object               # @.str.14
.L.str.14:
	.asciz	"realloop: %d %0.3g\n"
	.size	.L.str.14, 20

	.type	.L.str.15,@object               # @.str.15
.L.str.15:
	.asciz	"realprog: %d %0.3g\n"
	.size	.L.str.15, 20

	.type	.Lstr,@object                   # @str
.Lstr:
	.asciz	"OPTIMUM RESTART RESULTS"
	.size	.Lstr, 24

	.type	.Lstr.1,@object                 # @str.1
.Lstr.1:
	.asciz	"REALISTIC RESTART RESULTS -- Unlimited amount of CPUs"
	.size	.Lstr.1, 54

	.section	".note.GNU-stack","",@progbits
	.addrsig
